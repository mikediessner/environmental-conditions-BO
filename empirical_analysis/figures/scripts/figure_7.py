...
for i in range(len(labels)):
    ...
    for run in range(runs):
        ...
        mape = np.loadtxt(f"{directories[i]+filenames[i]}_mape_run{run+1}.csv", skiprows=1, delimiter=",")
        mapes[run, :] = mape[:, -1]
        
    all_mapes[i*runs:(i+1)*runs, :] = mapes
    all_axis_ranges[i*runs:(i+1)*runs] = axis_range
    axs[0, 0].scatter(axis_range, mapes[:, -1], label=labels[i])

# compute trendline
x = np.polyfit(all_axis_ranges, all_mapes[:, -1], 1)
trend = np.poly1d(x)
...
for i in range(len(labels)):
    ...
    all_mapes[i*runs:(i+1)*runs, :] = mapes
    all_axis_ranges[i*runs:(i+1)*runs] = axis_range
    axs[0, 1].scatter(axis_range, mapes[:, -1], label=labels[i])
...
for i in range(len(labels)):
    ...
    all_mapes[i*runs:(i+1)*runs, :] = mapes
    all_axis_ranges[i*runs:(i+1)*runs] = axis_range
    axs[1, 0].scatter(axis_range, mapes[:, -1], label=labels[i])
...
for i in range(len(labels)):
    ...
    all_mapes[i*runs:(i+1)*runs, :] = mapes
    all_axis_ranges[i*runs:(i+1)*runs] = axis_range
    axs[1, 1].scatter(axis_range, mapes[:, -1], label=labels[i])
