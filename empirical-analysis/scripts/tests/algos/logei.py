...
import torch
import numpy as np
from source.logei import envbo_experiment
from source.predict import make_predictions
from source.plot import give_scores, aggregate_mae, make_plot
from joblib import Parallel, delayed


def logei(func, noise, bounds, dynamic_dims, num_starts, num_samples, evals, runs, stepsize, num_tests, eval_stepsize, cores, name, start_str, walk_str):
    # paths
    data_dir = f"tests/{name}/"
    plot_dir = "tests/plots/"
    filename = f"observation-logei-{name}"
    ...
    for run in range(runs):
        walk = np.loadtxt(f"data/random_walks/{walk_str}_random_walk_run{run+1}.csv", delimiter=",")
        walk = torch.from_numpy(walk).reshape((-1, len(dynamic_dims)))
        random_walks.append(walk)

        start = np.loadtxt(f"data/starts/{start_str}_starts_run{run+1}.csv", delimiter=",")
    ...
    maes = aggregate_mae(runs, data_dir+filename)
