...
import torch
import numpy as np
from scripts.source.ei import envbo_experiment
...
dynamic_dims = [0, 3, 5]             # list of indices for dynamic dimensions
...
directory = "data/hartmann/multi_3/" # directory path
filename = "hartmann_multi_3"        # filename for output csv files
stepsize = [0.10, 0.10, 0.05]        # stepsize of random walk
...
for run in range(runs):
    walk = np.loadtxt(f"data/random_walks/3d_random_walk_run{run+1}.csv", delimiter=",")
